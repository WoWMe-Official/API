...
async def evaluate_identification(token: str, user_id: int) -> json:
    ...
    if not os.path.exists(rf"images\{user_id}\identification.jpeg"):
        raise HTTPException(
    ...
    if auth_level == 9:
        return FileResponse(rf"images\{user_id}\identification.jpeg")
    else:
        raise HTTPException(
...
async def evaluate_certification(token: str, user_id: int) -> json:
    ...
    if not os.path.exists(rf"images\{user_id}\certification.jpeg"):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if auth_level == 9:
        return FileResponse(rf"images\{user_id}\certification.jpeg")
    else:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="You do not have the necessary permissions to evaluate this account.",
